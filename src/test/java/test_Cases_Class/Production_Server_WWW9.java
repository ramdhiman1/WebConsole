...
public class Production_Server_WWW9 extends Base_Page {
	...
	@Test(priority = 2, groups = { "smoke",
			"Regression" }, description = "Log in to the Cloud Console on 'www9' to check machine status, install products, and verify their status.")
	public void Login_on_Cloud_Server_www9() throws InterruptedException, IOException {
		...
		Multiple_Login_ProductionServer login9 = new Multiple_Login_ProductionServer();
		login9.EnterUserName(username);
		login9.ClickOnNextBtn();
		login9.EnterPassword(password);
		login9.ClickOnSignBtn();
		...
		Policies_Page_ProductionServer dfPolicy9 = new Policies_Page_ProductionServer();
		dfPolicy9.clickAddPolicyButton();
		dfPolicy9.selectDropdownPolicy();
		dfPolicy9.enterPolicyName(randomPolicyName);
		dfPolicy9.EnableDFService();
		dfPolicy9.ClickonDropdwonDF("Enable (Install and use below settings)");
		dfPolicy9.EnableSoftwareUpdater();
		dfPolicy9.ClickonDropdwonSU("Enable (Install and use below settings)");
		dfPolicy9.selectanyapps();
		dfPolicy9.clickSaveButton();
		driver.get().get(p.getProperty("www9serverdownloadagent"));
		logToReport(Status.INFO, "Navigated to Cloud Agent Download Page For Download Cloud Agent Bootstraper");
		dfPolicy9.ClickOnInstallCloudAgentbtn(randomPolicyName);
		Thread.sleep(100000);
		getDriver().get(p.getProperty("www9computerspage"));

		// here Launch The VM and Install the Cloud Products

		Install_CloudPro_Vbox_Production iw9 = new Install_CloudPro_Vbox_Production();
		...
		iw9.copyInstallerUsingPsExec();
		Thread.sleep(20000); // Ensure files copied
		iw9.installApplication();
	...
	//	Thread.sleep(100000);
		...
		driver.get().get(p.getProperty("www9computerspage"));
		iw9.SearchMachine();
		iw9.selectOnlineWorkstation();
		iw9.SearchMachine();
		iw9.printAllProductStatuses();
		iw9.selectOnlineWorkstation();
		iw9.dfthawedAction();
